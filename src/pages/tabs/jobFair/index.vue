<template>
  <div class="preaching">
    ...
    <div class="list">
      ...
      <a class="item" v-for="(val, key) in list" :href="'/pages/jobFairDetail/main?acb330=' + val.acb330" :key="val.acb330">
        <div class="left">
          <div class="left-item">
            <div>{{val.time1}}</div>
          </div>
        ...
        <div class="right">
          <div class="ct-tit">
            <div class="ct-title">{{val.acb331}}</div>
          </div>
          <div class="mid">
            <image src="/static/img/position.png"></image>
            <div>{{val.acd200name}}</div>
          </div>
          <div class="bot">
            <div class="during">
  ...
  export default {
    ...
    methods: {
      ...
      getList() {
        ...
        this.$post('/service/business/college/jobfair/jobFairInfo/getJobFairByTime.xf', this.searchData).then(res => {
          this.loading = false
          this.pageBean = res.data.pageBean
          res.data.result.forEach(item => {
            item.time1 = dateFormat(item.acb333, 'dd')
            item.time2 = dateFormat(item.acb333, 'yyyy-MM')
            item.start = dateFormat(item.acb333, 'MM-dd hh:mm')
            item.end = dateFormat(item.acb334, 'MM-dd hh:mm')
            if (dateFormat(item.acb333, 'yyyy-MM-dd') === dateFormat(now, 'yyyy-MM-dd')) {
              item.tipCode = 1
              item.tip = '今天'
            } else if (dateFormat(item.acb333, 'yyyy-MM-dd') === dateFormat(new Date(now.getTime() + (1000 * 60 * 60 * 24)), 'yyyy-MM-dd')) {
              item.tip = '明天'
            } else {
              item.tip = ''
            }
